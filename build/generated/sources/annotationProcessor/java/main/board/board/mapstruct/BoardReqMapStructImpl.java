...
public class BoardReqMapStructImpl implements BoardReqMapStruct {
    ...
    public BoardEntity toEntity(BoardReqDto dto) {
        ...
        boardEntity.setFileList( boardFileReqDtoListToBoardFileEntityList( dto.getFileList() ) );
    ...
    protected List<BoardFileEntity> boardFileReqDtoListToBoardFileEntityList(List<BoardFileReqDto> list) {
        if ( list == null ) {
            return null;
        }

        List<BoardFileEntity> list1 = new ArrayList<BoardFileEntity>( list.size() );
        for ( BoardFileReqDto boardFileReqDto : list ) {
            list1.add( boardFileReqDtoToBoardFileEntity( boardFileReqDto ) );
        }
